...
void ts3plugin_infoData(uint64 serverConnectionHandlerID, uint64 id, enum PluginItemType type, char** data)
{
	...
	uint64 month_bytes_uploaded, month_bytes_downloaded, total_bytes_uploaded, total_bytes_downloaded, iconid, created, client_dbid;
	char* client_type;
	char* client_country;
	char* client_uid;
	...
	case PLUGIN_CLIENT:
		...
		if (ts3Functions.getClientVariableAsUInt64(serverConnectionHandlerID, (anyID)id, CLIENT_DATABASE_ID, &client_dbid) != ERROR_ok)
		{
			return;
		}
		//string
		...
		*data = (char*)malloc(INFODATA_BUFSIZE * sizeof(char));
		snprintf(*data, INFODATA_BUFSIZE, "\n Nickname: %s \n  Phonetic-Name: %s \nIP: %s \n Ping: %s \n Unique-ID: %s \n Database-ID: %I64u \n Total Connections: %i \n Client Type: %s \n Country: %s \n Idle-Time(ms): %s \n Unread Messages: %i \n Down-/Uploaded This Month: %I64u/%I64u B \n Down-/Uploaded Total: %I64u/%I64u B \n", client_nickname, phonetic_name, ip, ping, client_uid, client_dbid, totalconnection, client_type, client_country, idletime, unread_messages, month_bytes_downloaded, month_bytes_uploaded, total_bytes_downloaded, total_bytes_uploaded);
		break;
	default:
		break;
	}
