...
#ifdef EVIL_FEATURES
#define MAX_CHANNELS_FOR_OWNAGE 10
...
const char* ts3plugin_author() {
	return "Christian Goltz, Stefan Seering";
}

/* Plugin description */
const char* ts3plugin_description() {
	return "This plugin allows you to follow a user while he switches through channels. For the love menu just right click any name in the server view.  --  http://addons.teamspeak.com/directory/plugins/miscellaneous/Love.html  --  http://forum.teamspeak.com/showthread.php/59939-BETA-love-autofollower  --  https://sourceforge.net/projects/ts3love/";
}
...
void ts3plugin_infoData(uint64 serverConnectionHandlerID, uint64 id, enum PluginItemType type, char** data) {
	*data = (char*)malloc(INFODATA_BUFSIZE * sizeof(char));  /* Must be allocated in the plugin! */
	...
	switch (type) {
	case PLUGIN_SERVER:
		snprintf(*data, INFODATA_BUFSIZE, "TS3 Love Plugin <3 31337 The Cake is a Lie");
		break;
	case PLUGIN_CHANNEL:
		love = is_channel_lovely(serverConnectionHandlerID, id);
		snprintf(*data, INFODATA_BUFSIZE, "%sovely Channel (You %s automatically follow users into this Channel.)", love ? "L" : "Unl", love ? "will" : "won't");
		break;
	case PLUGIN_CLIENT:
		love = get_loved_user(serverConnectionHandlerID) == id;
		snprintf(*data, INFODATA_BUFSIZE, "%s (You %s automatically follow this User.)", love ? "Loved one" : "Just a friend", love ? "will" : "won't");
		break;
	default:
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	*/
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_LOVE_CLIENT, "Love this user <3", "");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_UNFOLLOW, "Unfollow this user", "");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CHANNEL, MENU_ID_FOLLOW_INTO_CHANNEL, "Follow into this Channel", "");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CHANNEL, MENU_ID_DONT_FOLLOW_INTO_CHANNEL, "Don't follow into this Channel", "");
#ifdef EVIL_FEATURES
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_OWN, "own", "");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_FORGIVE, "forgive", "");
#endif
